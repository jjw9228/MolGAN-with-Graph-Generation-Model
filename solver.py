...
from DIG.dig.ggraph.method.GraphDF.model import GraphFlowModel


class Solver(object):
    """Solver for training and testing StarGAN."""

    def __init__(self, config, config2=None, is_df=False):
    ...
    def build_model(self):
        ...
        self.G2 = GraphFlowModel(self.config2["model"])
        self.G2.to(self.device)

        self.g_optimizer = torch.optim.Adam(list(self.G.parameters())+list(self.V.parameters()),
                                            self.g_lr, [self.beta1, self.beta2])
        if self.is_df:
            self.g_optimizer = torch.optim.Adam(list(self.G2.parameters()) + list(self.V.parameters()),
                                            self.g_lr, [self.beta1, self.beta2])
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            a = torch.from_numpy(a).to(self.device).long()            # Adjacency. 16, 9, 9, 5 (batch, node, node, bond type)
                                                                      # 4??
            x = torch.from_numpy(x).to(self.device).long()            # Nodes.     16, 9, 5    (batch, node, node type)
                                                                      # C, O, N, F, padding
            a_tensor = self.label2onehot(a, self.b_dim)
            x_tensor = self.label2onehot(x, self.m_dim)
            a_tensor_df = a_tensor.permute(0, 3, 1, 2)
            x_tensor_df = x_tensor
            ...
            if not self.is_df:
                ...
                A = []
                X = []
                while len(A) != mols.size:
                    final_mol, pure_valid, num_atoms = self.G2.generate(self.config2["atom_list"], self.config2["temperature"], max_atoms = 9)
                    edge_array = self.data._genA(final_mol, connected=True, max_length=9)
                    while edge_array is None:
                        final_mol, pure_valid, num_atoms = self.G2.generate(self.config2["atom_list"], self.config2["temperature"], max_atoms = 9)
                        edge_array = self.data._genA(final_mol, connected=True, max_length=9)
                    edges_tensor = torch.tensor(edge_array, dtype=torch.int64).to(self.device)
                    edges_tensor = self.label2onehot(edges_tensor, self.b_dim)
                    nodes_tensor = torch.tensor(self.data._genX(final_mol, max_length=9), dtype=torch.int64).to(self.device)
                    nodes_tensor = self.label2onehot(nodes_tensor, self.m_dim)
                    A.append(edges_tensor)
                    X.append(nodes_tensor)
                edges_tensor = torch.stack(A)
                nodes_tensor = torch.stack(X)
                logits_fake, features_fake = self.D(edges_tensor, None, nodes_tensor)
                d_loss_fake = - torch.mean(logits_fake)
            ...
            if (i+1) % self.n_critic == 0:
                # Z-to-target
                if not self.is_df:
                    edges_logits, nodes_logits = self.G(z)# Postprocess with Gumbel softmax
                    (edges_hat, nodes_hat) = self.postprocess((edges_logits, nodes_logits), self.post_method)
                    logits_fake, features_fake = self.D(edges_hat, None, nodes_hat)
                    ...
                    fake_mols = []
                    while len(A) != mols.size:
                        final_mol, pure_valid, num_atoms = self.G2.generate(self.config2["atom_list"],
                                                                            self.config2["temperature"], max_atoms=9)
                        edge_array = self.data._genA(final_mol, connected=True, max_length=9)
                        while edge_array is None:
                            final_mol, pure_valid, num_atoms = self.G2.generate(self.config2["atom_list"],
                        ...
                        A.append(edges_tensor)
                        X.append(nodes_tensor)
                        fake_mols.append(final_mol)
                    edges_tensor = torch.stack(A)
                    nodes_tensor = torch.stack(X)
                    logits_fake, features_fake = self.D(edges_tensor, None, nodes_tensor)
                    g_loss_fake = - torch.mean(logits_fake)
                    rewardF = torch.from_numpy(self.reward(fake_mols)).to(self.device)
                    value_logit_fake,_ = self.V(edges_tensor, None, nodes_tensor, torch.sigmoid)
            ...
            if (i+1) % self.log_step == 0:
                ...
                if not self.is_df:
                    ...
                    m0, m1 = all_scores(fake_mols, self.data, norm=True)     # 'mols' is output of Fake Reward
